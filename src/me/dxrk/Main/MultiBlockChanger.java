...
public class MultiBlockChanger {
	...
	public void start(JavaPlugin plugin) {
				...
				if (!blockChanges.isEmpty()) {
					for (int i = 0; i < maxChanges; i++) {
						...
						*/
				        ...
				        ChunkSection cs = chunk.getSections()[blockVector.getY() >> 4];
				        if(cs == null) {
				        	cs = new ChunkSection(blockVector.getY() >> 4 << 4, !chunk.world.worldProvider.o()); 
			                chunk.getSections()[blockVector.getY() >> 4] = cs;
				        }
		...
		if (async) {
			bukkitTask = Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, runnable, 2L, tick);
			return;
		}
